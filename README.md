...
아르바이트 과정에서 전달받은 엑셀 파일을 기준으로 **500개의 외식·프랜차이즈 업체 대표전화 번호를 조사하는 업무**를 수행했습니다.
...
500개 업체를 대상으로 동일한 작업을 반복해야 했기 때문에 검색과 데이터 확인에 상당한 반복 작업이 발생했습니다.
...
자동화 프로그램을 사용하면서 한 번에 모든 업체를 처리하지 않고 **50개 단위로 나누어 실행**했습니다.
...
이 프로젝트에서는 ChatGPT(CODEX)를 개발 보조 도구로 활용했습니다.
